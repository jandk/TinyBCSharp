    ...
    : BlockDecoder(bytesPerBlock, bytesPerPixel)
{
    ...
    static ReadOnlySpan<uint> Partition2 =>
    [
    ...
    static ReadOnlySpan<uint> Partition3 =>
    [
    ...
    static ReadOnlySpan<byte> Anchor11 =>
    [
    ...
    static ReadOnlySpan<byte> Anchor21 =>
    [
        +3, +3, 15, 15, +8, +3, 15, 15,
        +8, +8, +6, +6, +6, +5, +3, +3,
        +3, +3, +8, 15, +3, +3, +6, 10,
    ...
    static ReadOnlySpan<byte> Anchor22 =>
    [
        15, +8, +8, +3, 15, 15, +3, +8,
    ...
    static ReadOnlySpan<byte> Weights2 => [0, 21, 43, 64];
    static ReadOnlySpan<byte> Weights3 => [0, 9, 18, 27, 37, 46, 55, 64];
    static ReadOnlySpan<byte> Weights4 => [0, 4, 9, 13, 17, 21, 26, 30, 34, 38, 43, 47, 51, 55, 60, 64];

    internal static uint Partition(int numPartitions, int partitionIndex)
    {
        return numPartitions switch
        {
            2 => Partition2[partitionIndex],
            3 => Partition3[partitionIndex],
            _ => 0
        };
    }

    internal static ReadOnlySpan<byte> Weights(int numIndexBits)
    {
        return numIndexBits switch
        {
            ...
            3 => Weights3,
            4 => Weights4,
            _ => throw new ArgumentOutOfRangeException(nameof(numIndexBits))
        };
    }
